...
//unsigned Node::NewLabel(const std::string& labelType)
unsigned Node::NewLabel()
{ 
    //cout << "Nova label criada: " << labelType << " L" << (labels + 1) << endl;
    return ++labels; 
}
    ...
    stmt(s) 
{
    ...
    before = NewLabel();
    after = NewLabel();
    //before = NewLabel("before WHILE");
    //after = NewLabel("after WHILE");
}

void While::Gen()
{
    cout << "L" << before << ':' << endl;  // Rótulo de início para a condição do loop
    Expression * n = Rvalue(expr);         // Avalia a condição antes de entrar no loop
    cout << "\tifFalse " << n->ToString() << " goto L" << after << endl;  // Sai do loop se a condição for falsa
    stmt->Gen();                           // Gera o código para o corpo do loop
    cout << "\tgoto L" << before << endl;  // Retorna para reavaliar a condição
    cout << "L" << after << ':' << endl;
